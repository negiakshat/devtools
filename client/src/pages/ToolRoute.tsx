...
function ToolHelp({ tool }: { tool: ToolDefinition }) {
  return <section className="tool-help" aria-labelledby="tool-help-title">
    <div className="help-main"><p className="eyebrow">Tool notes / quick reference</p><h2 id="tool-help-title">What this utility does</h2><p>{tool.help.what}</p><div className="help-protocol"><div><b>How it works</b><span>Your browser performs the requested deterministic operation after you run it.</span></div><div><b>Data handling</b><span>Text you paste or load from a local file stays in browser memory; this tool does not upload it to perform the transformation.</span></div></div><p className="reference-id">COMMAND / {tool.slug.toUpperCase()} · EXECUTION / BROWSER</p><div className="use-case-list">{tool.help.useCases.map((item) => <span key={item}>USE / {item}</span>)}</div></div>
    <div className="faq-block"><p className="eyebrow">Constraints & processing</p><div className="faq-list">{tool.help.faqs.map((faq) => <details key={faq.question}><summary>{faq.question}<span>+</span></summary><p>{faq.answer}</p></details>)}</div></div>
    <aside className="related-tools"><p className="eyebrow">Adjacent commands</p><h3>Next utilities</h3>{tool.related.map((slug) => { const related = toolBySlug[slug]; const Icon = related.icon; return <Link key={slug} href={`/${slug}`} onClick={() => trackToolEvent("related_tool_clicked", tool, { destination_slug: slug, source: "related_tools" })}><Icon size={15} />{related.name}<ArrowRight size={14} /></Link>; })}</aside>
  </section>;
}
